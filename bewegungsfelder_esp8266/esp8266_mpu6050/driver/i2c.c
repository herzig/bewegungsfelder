...
// set  write bit in i2c address
#define I2C_WRITE_BIT << 1

// set read bit in i2c address
#define I2C_READ_BIT << 1 | 1
...
/**
 * Write several bytes to i2c
 * unsigned char slave_addr: slave device address
 * unsigned char reg_addr: slave register address
 * unsigned char length: number of bytes to send
 * unsigned char const *data: data to write
 * return 0 on success, everything else is errors
 */
int i2c_writeBytes(unsigned char slave_addr, unsigned char reg_addr,
	...
	{
		i2c_stop();
		ets_uart_printf("i2c_writeBytes: no ack after slave addr: 0x%x\n", slave_addr);
		return -1;
	}
	...
	{
		i2c_stop();
		ets_uart_printf("i2c_writeBytes: no ack after reg addr: 0x%x\n", reg_addr);
		return -1;
	}
	...
	for (i = 0; i < length; ++i) {
		...
		{
			i2c_stop();
			ets_uart_printf("i2c_writeBytes: no ack after byte # %d data: 0x%x", i, *data);
			return -1;
		}
	}
...
/**
 * Read several bytes from i2c
 * unsigned char slave_addr: slave device address
 * unsigned char reg_addr: slave register address
 * unsigned char length: number of bytes to receive
 * unsigned char *data: read data is saved here
 * return 0 on success, everything else is errors
 */
int i2c_readBytes(unsigned char slave_addr, unsigned char reg_addr,
	...
	i2c_start();
	i2c_writeByte(slave_addr I2C_WRITE_BIT);
	if (!i2c_check_ack())
	{
		i2c_stop();
		ets_uart_printf("i2c_readBytes: no ack after slave addr: 0x%x\n", slave_addr);
		return -1;
	}
	...
	{
		i2c_stop();
		ets_uart_printf("i2c_readBytes: no ack after reg addr: 0x%x\n", reg_addr);
		return -1;
	}

	i2c_start();
	i2c_writeByte(slave_addr I2C_READ_BIT);
	if (!i2c_check_ack())
	{
		i2c_stop();
		ets_uart_printf("i2c_readBytes: no ack after slave-read addr: 0x%x\n", slave_addr I2C_READ_BIT);
		return -1;
	}
	...
	for (i = 0; i < length; ++i) {
		*data = i2c_readByte();
		...
		if (i < length-1)
			i2c_send_ack(1); // ack while more bytes are expected
		else
			i2c_send_ack(0); // nack for the last byte
	}
